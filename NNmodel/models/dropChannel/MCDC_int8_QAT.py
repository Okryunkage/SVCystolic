...
def symmetric_int8_fake_quant(
	x: torch.Tensor,
	reduce_dims: tuple[int, ...] | None =None,
	#Dimensions reduced when computing the quantization scale.
	#For Conv2d weights, (1,2,3) gives one scale per output channel.
...
class Int8QATConv2d(nn.Conv2d):
	def forward(self, x: torch.Tensor)-> torch.Tensor:
		x_q      =symmetric_int8_fake_quant(x)
		weight_q =symmetric_int8_fake_quant(self.weight, reduce_dims=(1,2,3),)
		return F.conv2d(x_q, weight_q, self.bias, self.stride, self.padding, self.dilation, self.groups,)

class Int8QATLinear(nn.Linear):
	def forward(self, x:torch.Tensor)-> torch.Tensor:
		x_q      =symmetric_int8_fake_quant(x)
		weight_q =symmetric_int8_fake_quant(self.weight, reduce_dims=(1,),)
...
class BayesianInt8ChannelCNN(nn.Module):
	def __init__(self, channel_drop_probs: tuple[float, float])-> None:
		...
		self.conv1 =Int8QATConv2d(1, 32, kernel_size=3, padding=1)
		self.drop_channel1 =nn.Dropout2d(p=p1)
		self.conv2 =Int8QATConv2d(32, 64, kernel_size=3, padding=1)
		self.classifier =Int8QATLinear(64*7*7, 10)
	def forward(self, x:torch.Tensor)-> torch.Tensor:
