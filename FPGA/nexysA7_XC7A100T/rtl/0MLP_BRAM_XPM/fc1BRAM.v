...
module fc1BRAM(
	...
	xpm_memory_spram#(
		.ADDR_WIDTH_A(wAddrW),
		.AUTO_SLEEP_TIME(0),
		.BYTE_WRITE_WIDTH_A(tile*wWidth),
		.ECC_MODE("no_ecc"),
		.MEMORY_INIT_FILE("fc1_w_tile.mem"),
		.MEMORY_INIT_PARAM(""),
		.MEMORY_OPTIMIZATION("true"),
		.MEMORY_PRIMITIVE("block"),
		.MEMORY_SIZE((tile*wWidth)*wDepth),
		.MESSAGE_CONTROL(0),
		.READ_DATA_WIDTH_A(tile*wWidth),
		.READ_LATENCY_A(2),
		.READ_RESET_VALUE_A("0"),
		.RST_MODE_A("SYNC"),
		.USE_MEM_INIT(1),
		.WAKEUP_TIME("disable_sleep"),
		.WRITE_DATA_WIDTH_A(tile*wWidth),
		.WRITE_MODE_A("read_first"))
		fc1w0(
		.clka(clk),.ena(1'b1),.wea(1'b0),
		.addra(waddr),
		.dina({(tile*wWidth){1'b0}}),.douta(wdata),
	...
	xpm_memory_spram#(
		.ADDR_WIDTH_A(bAddrW),
		.AUTO_SLEEP_TIME(0),
		.BYTE_WRITE_WIDTH_A(tile*bWidth),
		.ECC_MODE("no_ecc"),
		.MEMORY_INIT_FILE("fc1_b_tile.mem"),
		.MEMORY_INIT_PARAM(""),
		.MEMORY_OPTIMIZATION("true"),
		.MEMORY_PRIMITIVE("block"),
		.MEMORY_SIZE((tile*bWidth)*numTile),
		.MESSAGE_CONTROL(0),
		.READ_DATA_WIDTH_A(tile*bWidth),
		.READ_LATENCY_A(2),
		.READ_RESET_VALUE_A("0"),
		.RST_MODE_A("SYNC"),
		.USE_MEM_INIT(1),
		.WAKEUP_TIME("disable_sleep"),
		.WRITE_DATA_WIDTH_A(tile*bWidth),
		.WRITE_MODE_A("read_first"))
		fc1b0(
		.clka(clk),.ena(1'b1),.wea(1'b0),
		.addra(baddr),
		.dina({(tile*bWidth){1'b0}}),.douta(bdata),
		.rsta(1'b0),.regcea(1'b1),.sleep(1'b0),
		.injectsbiterra(1'b0),.injectdbiterra(1'b0),
		.sbiterra(),.dbiterra());
