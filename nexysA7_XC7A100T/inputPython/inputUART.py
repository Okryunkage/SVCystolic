...
UART_PORT_DEFAULT = "COM3"
UART_BAUD_DEFAULT = 1000000
...
class DigitDrawer:
	def __init__(self,root:tk.Tk):
		...
		#UART configuration widgets
		self.port_label =tk.Label(root,text="UART Port")
		self.port_label.grid(row=3,column=0,padx=5,pady=5,sticky="e")
		self.port_entry =tk.Entry(root,width=14)
		self.port_entry.insert(0,UART_PORT_DEFAULT)
		self.port_entry.grid(row=3,column=1,padx=5,pady=5,sticky="w")
		self.baud_label =tk.Label(root,text="Baud")
		self.baud_label.grid(row=3,column=2,padx=5,pady=5,sticky="e")
		self.baud_entry =tk.Entry(root,width=14)
		self.baud_entry.insert(0,str(UART_BAUD_DEFAULT))
		self.baud_entry.grid(row=3,column=3,padx=5,pady=5,sticky="w")
		#Sends the generated .mem contents as 784 raw UART bytes.
		self.send_uart_btn =tk.Button(root,text="Send UART",width=20,command=self.send_uart)
		self.send_uart_btn.grid(row=4,column=1,columnspan=2,padx=5,pady=8)
	...
	def read_mem_as_bytes(self,filename):
		tx_values =[]
		with open(filename,"r",encoding="utf-8") as f:
			for line_num,line in enumerate(f,start=1):
				...
				try:
					...
					raise ValueError(f"Invalid hex value at line {line_num}:{line}")
				if value<0 or value>255:
					raise ValueError(f"Value out of byte range at line {line_num}:{line}")
				tx_values.append(value)
		if len(tx_values) !=MNIST_SIZE*MNIST_SIZE:
			raise ValueError(f"Expected 784 bytes, but got {len(tx_values)} bytes")
		return bytes(tx_values)
	...
	def send_uart(self):
		...
		if serial is None:
			messagebox.showerror(
				...
				"pyserial is required for UART.\n\nInstall it with:\npython -m pip install pyserial")
			return
		'''
		port =self.port_entry.get().strip()
		baud_text =self.baud_entry.get().strip()
		if port =="":
			messagebox.showerror("UART Error","UART port is empty.")
			return
		try:
			baud =int(baud_text)
		except ValueError:
			messagebox.showerror("UART Error",f"Invalid baud rate: {baud_text}")
			return
		try:
			#First generate the latest .mem from the current canvas.
			#Then read the .mem file back and transmit each line as one raw byte.
			self.write_mem_file()
			tx_data =self.read_mem_as_bytes(MEM_FILENAME)
			with serial.Serial(port=port,baudrate=baud,timeout=1,write_timeout=2) as ser:
				#Small delay after opening the port.
				#Usually harmless for FPGA UART, and useful for some USB-UART chips.
				time.sleep(0.05)
				written =ser.write(tx_data)
				ser.flush()
			self.status_label.config(text=f"UART TX completed: {written} bytes @ {baud} baud")
			messagebox.showinfo("UART TX",f"Sent {written} bytes form {MEM_FILENAME}")
		except Exception as e:
			self.status_label.config(text="UART TX failed")
			messagebox.showerror("UART TX Error",str(e))
