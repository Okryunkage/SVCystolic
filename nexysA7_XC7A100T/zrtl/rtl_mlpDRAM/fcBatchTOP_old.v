...
module fcBatchAccTOP#(
	...
	//Example: 875 means 87.5%.
	output reg [31:0] accuracyPermille,
	...
	localparam [3:0] S_ACC_MUL     =4'd6;
	localparam [3:0] S_ACC_DIV     =4'd7;
	localparam [3:0] S_DONE        =4'd8;
	localparam [3:0] S_ERROR       =4'd9;

	reg [3:0] state;

	reg [(batchIDXwidth-1):0] batchIdx;

	//Registered multiply result for accuracy calculation.
	reg [63:0] accProduct;
	...
	always@(posedge clk or posedge rst)begin
		if(rst)begin
			...
			case(state)
				...
				//Step 1: multiply correctCount by 1000.
				//This separates the multiplier from the divider.
				S_ACC_MUL:begin
					accProduct <=correctCount*64'd1000;
					state <=S_ACC_DIV;
				end

				//Step 2: divide the registered product by batchSize.
				S_ACC_DIV:begin
					accuracyPermille <=accProduct/batchSize;
					state <=S_DONE;
				end
