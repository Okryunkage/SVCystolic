		...
		if(rst)begin
			...
			if(start)begin
				for(n=0;n<number;n=n+1)begin
					...
					if(inElement<=0) begin
						shifted =0;
					end
					else begin
						// >> : logical shift
						// >>>: arithmetic shift
						shifted =(inElement+(1<<(shift-1)))>>>shift;
					end

					//(inElement+(1<<(shift-1))) for rounding, then shift right
					if(shifted<0) begin
						clipped ={outWidth{1'b0}};
					end
					else if(shifted>((1<<outWidth)-1)) begin
						clipped ={outWidth{1'b1}};
					end
					else begin
						clipped =shifted[(outWidth-1):0];
					end
