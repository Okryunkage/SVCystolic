...
module finalTOP#(
	...
	//Names kept as in your current XDC/top.
	output wire         UARTRX,
	input  wire         UARTTX,
	...
	//Result-valid latch for 8-seg display.
	reg accResultValid;
	...
	wire [31:0] correctAscii4;

	bin2decASCII1#(
		.binWidth(32),
		.digits(4))
		u_correctBin2DecASCII(
		.bin(correctCount),
		.asciiFlat(correctAscii4)
	);
	...
	always@(*)begin
		...
		else if(accResultValid)begin
			//Display: "   Cxxxx"
			//xxxx = correctCount.
			//Example: C0625 means 625 correct predictions.
			segAsciiData ={
				`ASCII_SPACE,
				`ASCII_SPACE,
				`ASCII_SPACE,
	...
	assign LEDarr[8] =accResultValid;            // correctCount result is valid

	assign LEDarr[9]  =fcError;
	assign LEDarr[10] =batchError;
	assign LEDarr[11] =labelValueError;
